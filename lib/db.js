...
class LimitDBRedis extends EventEmitter {
  ...
  take(params, callback) {
    ...
    this.redis.take(`${params.type}:${params.key}`,
      ...
      (err, [new_remaining, enough_tokens, current_timestamp_ms]) => {
        if (err) {
          return callback(err);
        }

        const remaining = parseInt(new_remaining, 10);
        return callback(null, {
          conformant: parseInt(enough_tokens, 10) ? true : false,
          remaining,
          reset: this.calculateReset(bucketKeyConfig, remaining, parseInt(current_timestamp_ms, 10)),
          limit: bucketKeyConfig.size,
          delayed: false
        });
      });
  ...
  put(params, callback) {
    ...
    this.redis.put(key,
      ...
      bucketKeyConfig.size,
      Math.floor(bucketKeyConfig.ttl || this.globalTTL),
      (err, [new_remaining, current_timestamp_ms]) => {
        if (err) {
          return callback(err);
        }
